...
`cast` is a Bun app for local speech-to-speech conversation on Apple Silicon. `web/src` owns the Vite + React browser UI, microphone capture, playback, settings, and WebSocket client behavior. `web/src/voice-agent-config.ts` owns browser-side agent instructions. `server/voice-server.mjs` owns runtime composition across static serving, WebSocket sessions, STT, LLM, tools, TTS, cancellation, and turn lifecycle. AI prompt and tool boundaries live in `server/ai`, following the `prompts` and `tools` split. Focused voice runtime modules live in `server/voice`; keep web gating, tool loops, speech cleanup, pronunciation normalization, turn runtime, session history, logging, wire encoding, and worker adapters there instead of growing the runtime file. Native STT/VAD code lives in `native`; local model artifacts live in `models` and should not be treated as application logic. Architecture boundaries live in `docs/ARCHITECTURE.md`.
...
## Maintainability
Long term maintainability is a core priority. Before adding local logic, check whether the behavior belongs in an existing focused module under `server/voice` or should be extracted into a new one. Duplicate logic across multiple files is a code smell. Do not preserve old fallback paths when the product has a single intended runtime path. Avoid regex-like easy fixes when the behavior needs durable product logic; prefer structured routing, explicit state, validated data shapes, focused policy modules, and tests that describe the intended contract. Do not add comments as a substitute for clear structure, names, tests, or docs. When a code change alters runtime boundaries, ownership, flow, tool routing, model behavior, configuration, or voice pipeline invariants, update `docs/ARCHITECTURE.md` in the same change.
