...
describe("Russian speech normalization", () => {
	...
	it("adds stress marks for known Russian pronunciation failures", () => {
		expect(
			...
			"Создания ви́джетов для цифровы́х каналов и цифрово́й платформы. Текстовые ме́ссенджеры. Прове́рю. Обнови́м. А́дрес и адреса. На автопило́те, без ударе́ния, с зада́чами, в багаже́, от ве́са, Ура́льские Авиали́нии.",
		);
		...
		expect(normalizeRussianSpeechText("Девять задач готовы.")).toBe(
			"Де́вять задач готовы.",
		);
		expect(
			normalizeRussianSpeechText(
